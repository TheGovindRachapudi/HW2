...
```{r}
library(tidyverse)
maac <- read.csv("C:\\Users\\User\\Desktop\\DATA product Class\\MAACS\\MAACS\\maacs.csv.gz")
...
Therefore, I wrote a simple function to see whether the slope is increasing at a positive rate or not. If that is the case, then the function will output TRUE in which the given statement will be proven correct according to the data.
...
1.   I created a scatterplot using ggplot to visualize the trend of the increase age with the correlation of fev1. The plot showed that as age increases fev1 also increases at significant rate.
...
3.  For the third and final supporting premise I used the correlation test function in R with age against fev1 to quantify whether there is a positive correlation between both of the variables or not. To no surprise, there is a significant positive correlation of .805 which indicates that the statement in the beginning of the assignment in indeed true.
